...
{
    class AccelleratingBullets : CustomCard
    {
        ...
        public override void OnAddCard(Player player, Gun gun, GunAmmo gunAmmo, CharacterData data, HealthHandler health, Gravity gravity, Block block, CharacterStatModifiers characterStats)
        {
            ActivatorMono.Add(player, (obj, i) =>
            {
                ...
                {
                    moveTransform.velocity *= 2;
                }
            });

            Type type = typeof(AccelleratingShots);

            float defaultTime = 0.5f;

            if (!AccelleratingShots.time.ContainsKey(player.playerID)) AccelleratingShots.time.Add(player.playerID, defaultTime);
            else AccelleratingShots.time[player.playerID] /= 2f;
            ...
            AccelleratingShots.time[player.playerID] = defaultTime;

            GameObject obj = new GameObject("AccelleratingEffect", type);
        ...
        {
            return "Accellerating Bullets";
        }
        protected override string GetDescription()
        {
            return "<color=#9aa355>TIMED</color>\n" + "Your bullets speed up every 0.5s";
        }
    ...
    public class AccelleratingShots : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            {
                transform.parent.gameObject.AddComponent<AccelleratingShots>();
                return;
            }
            else
            {
                foreach (var obj in GetComponentsInChildren<AccelleratingShots>().Where(bullet => bullet != this))
                {
        ...
        public void SpeedUp(float newtime)
        {
            this.ExecuteAfterSeconds(newtime / owner.data.weaponHandler.gun.projectielSimulatonSpeed, () =>
            {
                SpeedUp(newtime);
